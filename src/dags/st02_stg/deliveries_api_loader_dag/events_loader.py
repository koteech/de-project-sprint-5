...
class deliveriesOriginRepository:
    ...
    def list_deliveries(self, delivery_threshold: int, limit: int) -> List[deliveryObj]:
        ...
        params = {
            ...
            'sort_direction': 'asc',
            'limit': limit,
            'offset': delivery_threshold
        }

        response = requests.get('https://d5d04q7d963eapoepsqr.apigw.yandexcloud.net/deliveries', params=params,
                                headers=headers)
        objs = response.json()
        keys = [0, 1]
        print(objs)
...
class deliveryDestRepository:
    ...
    def insert_delivery(self, conn: Connection, delivery: dict) -> None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                    INSERT INTO stg.stg_deliveries(object_value)
                    VALUES (%(object_value)s)
                    ;
                """,
                {
                    "id": delivery[0],
                    "object_value": delivery[1]
                },
            )
