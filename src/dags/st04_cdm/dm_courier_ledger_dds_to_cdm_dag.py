...
def st04_01_de_project_sprint5_courier_ledger_dds_to_cdm_dag():
    ...
    dwh_pg_connect = psycopg2.connect(
        host=PG_WAREHOUSE_CONNECTION["host"],
        port=PG_WAREHOUSE_CONNECTION["port"],
        dbname=PG_WAREHOUSE_CONNECTION["database"],
        user=PG_WAREHOUSE_CONNECTION["user"],
        password=PG_WAREHOUSE_CONNECTION["password"],
    ...
    def load_courier_ledger():
        sql_query = """
                               ...
                               from (
                                   ...
                                   where settlement_year=extract(  year from now())::INT and settlement_month=extract(month from now())::INT-1
                       """

        # Создаем курсор для выполнения SQL-запроса.
        cursor = dwh_pg_connect.cursor()

        # Выполняем SQL-запрос.
        cursor.execute(sql_query)

        # Фиксируем изменения и закрываем курсор и соединение.
        dwh_pg_connect.commit()
        cursor.close()
